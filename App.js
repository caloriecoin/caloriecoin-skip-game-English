...
const App = ({navigation}) => {
  ...
  useEffect(() => {
    if (loading >= 1.0) clearInterval(timerId.current);
  }, [loading]);
      ...
      <LoginView
        isLogin={async token => {
          ...
          if (token) {
            ...
            if (userData) {
              const callUserData = await axios.get(
                `https://caloriecoin.herokuapp.com/api/user/getUserAndWallet/${userData.id}`,
              );
          ...
          RNKakaoLogins.login()
            .then(response => {
              RNKakaoLogins.getProfile().then(async response => {
                if (response) {
                  const callUserData = await axios.get(
                    `https://caloriecoin.herokuapp.com/api/user/getUserAndWallet/${response.id}`,
                  );
    ...
    return (
      <>
        <Tab.Navigator
          screenOptions={({route}) => ({
            tabBarIcon: ({focused}) => (
              <TabBarIcon name={route.name} focus={focused} />
            ),
