...
const BLEconnectionView = ({navigation}) =>{ 
    ...
    return <View style={{flex:1, justifyContent:'center', alignItems:'center', backgroundColor:'#fff'}}>
        ...
        {connection === 'connect' && <TouchableOpacity style={styles.connect} onPress={()=>{dispatch(setSkippingConnection('disconnect'));}}>
            <Text style={{fontSize:18,margin:24, color:'white', fontFamily:'SUIT-Bold'}}>Device not connected <Text style={{fontSize:14, fontFamily:'SUIT-SemiBold'}}>({connectedMacAddr})</Text></Text>
        ...
        {connection === 'disconnect' && <TouchableOpacity style={styles.disconnect} disabled={true}> 
            <Text style={{fontSize:18,margin:24, color:'white'}}>Disconnection</Text>
        </TouchableOpacity>}
        {connection === 'connecting' && <TouchableOpacity style={styles.connecting} disabled={true}> 
            <Text style={{fontSize:18,margin:24, color:'white'}}>Waiting for device to connect</Text>
        </TouchableOpacity>}
        <Text style={{fontFamily:'SUIT-Bold', fontSize:24}}>device list</Text>
        <Text style={{fontFamily:'SUIT-Regular', marginTop:8}}>If you don't see the device list</Text>
        <Text style={{fontFamily:'SUIT-Regular', marginTop:8}}>Please check 'App settings > Permission > Location permission' !!</Text>
        <FlatList
            data={deviceList}
