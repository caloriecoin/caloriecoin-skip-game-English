      ...
      <View style={styles.container}>
        <View style={styles.topContainer}>
          <Text style={styles.datetime}>
            {format(slector?.timestamp, 'yyyy/MM/dd  HH:mm')}
          </Text>
          <Text style={styles.scoreWrap}>
            <Text style={{color: '#ff2c4b', fontSize: 16}}>
              {slector?.myScore}
            </Text>{' '}
            :{' '}
            <Text style={{color: '#005fff', fontSize: 16}}>
              {slector?.targetScore}
            </Text>
          </Text>
          <Text
            style={
              slector?.battleResult == 0
                ? styles.drawText
                : slector?.battleResult == 1
                ? styles.winnerText
                : styles.loserText
            }>
            {slector?.battleResult == 0
